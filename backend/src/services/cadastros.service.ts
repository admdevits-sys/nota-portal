...
export async function getUsuarios(page: number, pageSize: number) {
...
export async function updateUsuario(usuarioId: string, input: UsuarioUpdateInput) {
  if (input.email) {
    const existing = await prisma.usuarios.findUnique({ where: { email: input.email } });
  ...
  if (input.senha) data.senha_hash = await argon2.hash(input.senha);
...
export async function deactivateUsuario(usuarioId: string) {
  return prisma.usuarios.update({
    where: { PK_usuario_id: usuarioId },
    data: { ativo: false },
  });
...
export async function activateUsuario(usuarioId: string) {
  return prisma.usuarios.update({
    where: { PK_usuario_id: usuarioId },
    data: { ativo: true },
  });
...
export async function deleteUsuario(usuarioId: string) {
  ...
  if (importacoesCount > 0) {
    ...
    (err as any).code = "USUARIO_COM_IMPORTACOES";
    throw err;
  }
...
export async function getEmpresas(page: number, pageSize: number, q?: string) {
  const skip = (page - 1) * pageSize;
  const where = q
    ? {
        OR: [
...
export async function getEmpresaById(PK_empresa_id: string) {
  return prisma.empresas.findUnique({ where: { PK_empresa_id } });
}

export async function createEmpresa(input: EmpresaCreateInput) {
  ...
  if (existing) {
    ...
    (err as any).code = "EMPRESA_JA_EXISTE";
    throw err;
  }
...
export async function updateEmpresa(PK_empresa_id: string, input: EmpresaUpdateInput) {
  const data: any = { ...input };
  if (input.cnpj_cpf) data.cnpj_cpf = input.cnpj_cpf.replace(/[^0-9]/g, "");
  return prisma.empresas.update({ where: { PK_empresa_id }, data });
}

export async function deleteEmpresa(PK_empresa_id: string) {
  ...
  if (linked > 0) {
    ...
    (err as any).code = "EMPRESA_COM_NOTAS";
    throw err;
  }
  return prisma.empresas.delete({ where: { PK_empresa_id } });
}

export async function getProdutos(page: number, pageSize: number, q?: string) {
  const skip = (page - 1) * pageSize;
  const where = q
    ? {
        OR: [
...
export async function getProdutoById(PK_produto_id: string) {
  return prisma.produtos_cadastrados.findUnique({ where: { PK_produto_id } });
}

export async function createProduto(input: ProdutoCreateInput) {
  ...
  if (existing) {
    ...
    (err as any).code = "PRODUTO_JA_EXISTE";
    throw err;
  }
...
export async function getServicoById(PK_servico_id: string) {
  return prisma.servicos_cadastrados.findUnique({ where: { PK_servico_id } });
}

export async function createServico(input: ServicoCreateInput) {
  ...
  if (existing) {
    ...
    (err as any).code = "SERVICO_JA_EXISTE";
    throw err;
  }
...
export async function updateServico(PK_servico_id: string, input: ServicoUpdateInput) {
  return prisma.servicos_cadastrados.update({ where: { PK_servico_id }, data: input });
}

export async function deleteServico(PK_servico_id: string) {
  ...
  if (linked > 0) {
    ...
    (err as any).code = "SERVICO_COM_ITENS";
    throw err;
  }
