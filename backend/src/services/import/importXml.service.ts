...
import { createLogSistema } from "../logs-sistema.service.js";

export type ImportXmlInput = {
  usuarioId: string;
  nomeArquivo: string;
...
type ServicoState = {
  ...
  deducoes: string;
  // Campos extras NFSe
  desconto_incond?: string;
  total_deducoes?: string;
  total_beneficio?: string;
  base_calculo?: string;
  aliquota?: string;
  retencao?: string;
  versao_aplicacao?: string;
  ambiente_gerador?: string;
  situacao_nfse?: string;
  ...
  valor_retido_irrf?: string;
  valor_retido_csll?: string;
  valor_retido_cp?: string;
};
...
type NoteState = {
  ...
  emitente?: EmpresaState;
  destinatario?: EmpresaState;
  itens: ItemState[];
  servicos: ServicoState[];
  duplicatas: DuplicataState[];
  transporte?: TransporteState;
  impostos: ImpostoState;
};
...
async function readFileAsString(filePath: string): Promise<string> {
  ...
  let content = "";
  for await (const chunk of rs) content += chunk;
  return content;
}
...
const normalizeDecimal = (value: any, precision = 2): string => {
  if (value == null) return (0).toFixed(precision);
...
const normalizeQuantity = (value: any): string => {
...
const getAttributeValue = (attributes: Record<string, unknown> | undefined, name: string) => {
...
export async function importXmlService(
  ...
  const erroLog: Array<{ erro: string; tag?: string | null; linha?: number | null }> = [];
  let totalNotasDetectadas = 0;
  let processed = 0;
  const pendingNotePromises: Promise<void>[] = [];
  let rawXmlContent = "";

  try {
    await pipeline(input.stream, fs.createWriteStream(tmpPath));
    const digest = await sha256OfFile(tmpPath);
    rawXmlContent = await readFileAsString(tmpPath);
      ...
      const importacao = await prisma.importacoes.create({
        data: {
          ...
          tamanho_arquivo_bytes:
            input.tamanhoBytes ?? BigInt(fs.statSync(tmpPath).size),
          status: "PROCESSANDO",
          data_hora_inicio: new Date(),
        },
    ...
    await createAuditLog({
      ...
      endereco_ip: input.ip ?? "unknown",
      agente_usuario: input.userAgent ?? "unknown",
      detalhes: {
        nome_arquivo: input.nomeArquivo,
        hash_arquivo: digest,
      },
    ...
    await createLogSistema({
      fk_usuario_id: input.usuarioId,
      modulo: "XML",
      acao: "IMPORTACAO_XML_INICIADA",
      tabela_afetada: "importacoes",
      registro_afetado_id: importacaoId,
      descricao: `Importação de XML iniciada: ${input.nomeArquivo}`,
      detalhes: {
        nome_arquivo: input.nomeArquivo,
    ...
    let tipoRoot: "NFE" | "NFSE" | null = null;

    let currentNote: NoteState | null = null;
    let currentDet: ItemState | null = null;
    let currentService: ServicoState | null = null;
    let currentText = "";
    let currentTextTag: string | null = null;
    let tagStack: string[] = [];

    const isEmitContext = (parentTag: string) =>
      ["emit", "prest", "emitente", "prestador", "remet", "enderemit", "endernac", "ender"].includes(parentTag);
    const isDestContext = (parentTag: string) =>
      ["dest", "toma", "destinatario", "tomador", "tomaor", "enderdest", "endertoma", "enderdest", "ender"].includes(parentTag);

    const getEmpresaId = async (tx: Prisma.TransactionClient, empresa?: EmpresaState) => {
      if (!empresa || !empresa.cnpj_cpf) return null;
    ...
    const upsertProduto = async (tx: Prisma.TransactionClient, item: ItemState) => {
      if (!item.codigo_produto) return null;
    ...
    const upsertServico = async (tx: Prisma.TransactionClient, service: ServicoState) => {
      if (!service.codigo_servico) return null;
    ...
    const persistNoteInSingleTx = async (note: NoteState, tagFechamento: string, rawXml: string) => {
...
if (!note.chave_acesso || note.chave_acesso === null) {
        ...
        await tx.notas_fiscais.create({
          data: {
            ...
            fk_importacao_id: importacaoId as string,
            ...
            xml_bruto_json: rawXml,
          },
        });

        if (note.itens.length) {
          const itens = await Promise.all(
            note.itens.map(async (item) => ({
        ...
        if (note.servicos.length) {
          const services = await Promise.all(
            note.servicos.map(async (service) => ({
              fk_nota_fiscal_id: notaId,
    ...
    await new Promise<void>((resolve, reject) => {
      parser.on("opentag", (node: sax.Tag) => {
        ...
        if (!tipoRoot) {
          if (tag === "nfe" || tag === "nfeproc" || tag === "nep") tipoRoot = "NFE";
          else if (tag === "nfse" || tag === "compnfse" || tag === "rps" || tag === "infrps") tipoRoot = "NFSE";
        }
        ...
        if (tipoRoot === "NFE" && (tag === "infnfe" || tag === "nfe")) {
          const id = getAttributeValue(node.attributes as Record<string, unknown>, "id");
          currentNote = {
            tipo_documento: "NFE",
      ...
      parser.on("closetag", (name: string) => {
        ...
        if (currentNote) {
          if (currentNote.tipo_documento === "NFE") {
            if (t) {
              if (tag === "chnfe") currentNote.chave_acesso = t;
              else if (tag === "nnf") currentNote.numero_documento = t;
              else if (tag === "dhemi" || tag === "demi" || tag === "demi") {
                const d = parseDate(t);
                if (d) currentNote.data_emissao = d;
              } else if (tag === "cnpj" || tag === "cpf") {
                if (isEmitContext(parentTag)) currentNote.documento_emitente = t;
                else if (isDestContext(parentTag)) currentNote.documento_destinatario = t;
                else if (!currentNote.documento_emitente || currentNote.documento_emitente === "0") currentNote.documento_emitente = t;
                else if (!currentNote.documento_destinatario) currentNote.documento_destinatario = t;
              } else if (tag === "xnome") {
                if (isEmitContext(parentTag)) currentNote.nome_emitente = t;
                else if (isDestContext(parentTag)) currentNote.nome_destinatario = t;
                else if (!currentNote.nome_emitente) currentNote.nome_emitente = t;
                else if (!currentNote.nome_destinatario) currentNote.nome_destinatario = t;
              } else if (tag === "vnf" || tag === "vprod") currentNote.valor_total = t;
              else if (tag === "vtottrib") currentNote.total_impostos = t;
            }

            if (currentDet && t) {
              if (tag === "nitem") currentDet.numero_item = parseInt(t, 10) || 0;
              else if (tag === "cprod") currentDet.codigo_produto = t;
              else if (tag === "xprod") currentDet.descricao = t;
              else if (tag === "ncm") currentDet.ncm = t;
              else if (tag === "cfop") currentDet.cfop = t;
              else if (tag === "qcom") currentDet.quantidade = t;
              else if (tag === "vuncom") currentDet.valor_unitario = t;
              else if (tag === "vprod") currentDet.valor_total = t;
            }
          } else {
            if (t) {
              if (tag === "id") currentNote.chave_acesso = t.replace(/^(nfs|nfseref)/i, "").trim();
              else if (tag === "nnfse" || tag === "ndfse" || tag === "ndps") currentNote.numero_documento = t;
              ...
              } else if (tag === "vservprest" || tag === "vserv" || tag === "vtotal" || tag === "vnf" || tag === "vliq") currentNote.valor_total = t;
              else if (tag === "vtottrib" || tag === "vissqn") currentNote.total_impostos = t;
            }

            if (currentService && t) {
              if (tag === "cservico" || tag === "codservico" || tag === "ctribnac") currentService.codigo_servico = t;
              else if (tag === "xservico" || tag === "xdescserv" || tag === "xdescservico") {
                if (!currentService.descricao || currentService.descricao === "") currentService.descricao = t;
              }
              else if (tag === "vservprest" || tag === "vserv") currentService.valor_servico = t;
              else if (tag === "vissqn" || tag === "viss") currentService.valor_issqn = t;
              else if (tag === "vdeducao" || tag === "vded" || tag === "vdr") currentService.deducoes = t;
            }
          }

          if (currentNote.transporte && t) {
            if (tag === "modal" || tag === "modfrete") currentNote.transporte.modal = t;
            else if (tag === "placa") currentNote.transporte.placa = t;
            else if (tag === "uf") currentNote.transporte.uf = t;
            else if (tag === "xnome") currentNote.transporte.nome_transportador = t;
            else if (tag === "cnpj" || tag === "cpf") currentNote.transporte.cnpj_cpf_transportador = t;
            else if (tag === "xmun" || tag === "cidadeorigem") currentNote.transporte.cidade_origem = t;
            else if (tag === "cmun" || tag === "cidadedestino") currentNote.transporte.cidade_destino = t;
            else if (tag === "vfrete") currentNote.transporte.valor_frete = t;
          }

          if (currentNote.duplicatas.length && t) {
            const dup = currentNote.duplicatas[currentNote.duplicatas.length - 1];
            if (tag === "ndup" || tag === "numeroduplicata") dup.numero_duplicata = t;
            else if (tag === "dvenc" || tag === "datavencimento") {
              const d = parseDate(t);
              if (d) dup.data_vencimento = d;
            } else if (tag === "vdup" || tag === "valordup" || tag === "valor") dup.valor = t;
          }

          if (t) {
            if (tag === "vbc" && parentTag === "icmstot") currentNote.impostos.icms_base = t;
            else if (tag === "vicms") currentNote.impostos.icms_valor = t;
            else if (tag === "vpis") currentNote.impostos.pis_valor = t;
            else if (tag === "vcofins") currentNote.impostos.cofins_valor = t;
            else if (tag === "vipi") currentNote.impostos.ipi_valor = t;
            else if (tag === "vissqn") currentNote.impostos.issqn_valor = t;
            else if (tag === "vtottrib" || tag === "totaltributos") currentNote.impostos.total_tributos = t;
          }
        }

        if (tag === "det" && currentNote?.tipo_documento === "NFE" && currentDet) {
          if (!currentDet.descricao) currentDet.descricao = "";
        ...
        if ((tag === "infnfe" || tag === "nfe") && currentNote?.tipo_documento === "NFE") {
          const snapshot: NoteState = JSON.parse(JSON.stringify(currentNote));
          currentNote = null;
          currentText = "";
          currentTextTag = null;

          pendingNotePromises.push(
            persistNoteInSingleTx(snapshot, tag, rawXmlContent).catch((e: any) => {
        ...
        if (
          (tag === "infnfse" || tag === "infrps" || tag === "nfse") &&
    ...
    const statusFinal =
      erroLog.length === 0 && processed === totalNotasDetectadas
        ? "CONCLUIDO"
        : processed > 0
        ? "PARCIAL"
        : "FALHOU";
    ...
    await prisma.importacoes.update({
      where: { PK_importacao_id: importacaoId as string },
      data: {
    ...
    const acaoLog = statusFinal === "CONCLUIDO"
      ? "IMPORTACAO_XML_CONCLUIDA"
      : statusFinal === "PARCIAL"
      ? "IMPORTACAO_XML_PARCIAL"
      : "IMPORTACAO_XML_FALHOU";

    const descricaoLog = statusFinal === "CONCLUIDO"
      ? `Importação concluída: ${processed} notas fiscais processadas`
      : statusFinal === "PARCIAL"
      ? `Importação parcial: ${processed}/${totalNotasDetectadas} notas processadas`
      : "Importação falhou";
    ...
    return {
      importacaoId: importacaoId as string,
      status: statusFinal,
    ...
    if (importacaoId) {
      ...
      await createLogSistema({
        fk_usuario_id: input.usuarioId,
        modulo: "XML",
        acao: "IMPORTACAO_XML_FALHOU",
        tabela_afetada: "importacoes",
        registro_afetado_id: importacaoId,
        descricao: `Erro na importação: ${err?.message ?? "Falha estrutural no XML"}`,
        detalhes: {
          error: err?.message,
          cleanupErrors,
        },
