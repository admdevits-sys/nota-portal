...
import {
  faturamentoTotalController,
  fluxoCaixaController,
  impostosResumoController,
  notasController,
  statusCountController,
  topClientesController,
  topProdutosController,
} from "../controllers/dashboard.controller.js";

export async function dashboardRoutes(app: FastifyInstance) {
  const preHandler = [authenticate, authorize(["ADMIN", "OPERADOR", "AUDITOR"]) as any];

  app.get(
    "/dashboard/importacoes/status-count",
    { preHandler },
    statusCountController
  );

  app.get(
    "/dashboard/faturamento/total",
    { preHandler },
    faturamentoTotalController
  );
