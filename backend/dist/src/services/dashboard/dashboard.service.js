...
export async function getImportacoesStatusCount() {
    ...
    const map = {};
    for (const r of rows)
        map[r.status] = r._count.PK_importacao_id;
    return map;
}
...
export async function getNotasPaginadas(input) {
    ...
    if (input.data_ini || input.data_fim) {
        where.data_emissao = {};
        if (input.data_ini)
            where.data_emissao.gte = new Date(input.data_ini);
        if (input.data_fim)
            where.data_emissao.lte = new Date(input.data_fim);
    }
